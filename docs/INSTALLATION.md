# Installing LMAS 1.6.3

LMAS requires Python 3.11 or newer.

## Recommended release-kit installation

Activate the Python or Conda environment in which LMAS should be installed, open a terminal in the extracted `LMAS_1_6_3` folder, and run:
...
```bash
python -m pip install ./wheels/lmas-1.6.3-py3-none-any.whl
```
...
Expected version: `LMAS 1.6.3`.

## Cartography

LMAS includes compact offline boundary vectors for map underlays. Coast, country, state/province, and United States county lines require no separate map package, internet connection, or first-use download. Cartopy is optional and is not required for the boundary maps included in LMAS 1.6.3.
